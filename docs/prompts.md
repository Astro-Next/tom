...
The exact prompts passed to each `claude -p` subprocess. The launcher fills the `{placeholders}` — the issue or PR number(s) and the `{project-id}` (from `.tom/settings.json`, used in the attachment cache path) — and starts the agent in the correct working directory — see [Architecture — Launching agents](architecture.md#launching-agents). The launcher, working directory, and how each agent's JSON output is consumed are described on the agent pages and in the architecture doc; this file is the prompt text itself.

Each prompt is self-contained: a fresh Claude Code session reads only the prompt, so the prompts name no internal components and rely on no outside context. They share one contract:

- **Full tools, real work.** The agent reads files, greps the repo, runs `gh` to pull issue/PR data, searches the web, runs tests, and — for the dev agent — writes code. It does whatever it needs to do the task well.
- **Return a result; don't perform workflow actions.** The agent never changes labels, posts issue or PR comments, creates or merges PRs, closes issues, or creates new issues. It returns a JSON object describing what to do; those actions happen outside the session.
- **Final output is one JSON object** matching the schema at the end of the prompt.

---

## Dev prompt

Fills `{issue_number}`. Runs in a git worktree already checked out on the correct branch — a new branch for a new issue, or the existing PR branch when a PR already exists. See [Dev agent](agents/dev.md).
...
Fills `{pr_number}` and `{issue_number}`. Runs in a git worktree already checked out on the PR's head branch. See [Review agent](agents/review.md).
...
Fills `{issue_number}`. Runs in the project root (read-only judgment — it writes no code). See [PM agent](agents/pm.md).
...
Fills `{pr_numbers}` and `{issue_numbers}` — the merged PRs and closed issues in scope for this retrospective. Runs in the project root. See [Analyst agent](agents/analyst.md).
...
Read the merged diffs as your primary input — the most valuable findings are the ones review did not catch. Read project context (CLAUDE.md, CONVENTIONS.md, docs/index.md) and the surrounding source as needed. If an issue references cached attachments, read them from `/tmp/tom-{project-id}/cache/<issue_number>/` if present; do not download anything.
